    ...
    async def search_logs(
        ...
        async with self.db.session() as session:
            ...
            for row in rows:
                ...
                body_text = body_jsonb.get("string_value", "") if isinstance(body_jsonb, dict) else str(body_jsonb)

                # Parse attributes JSONB into list of SpanAttribute
                attributes_dict = json.loads(row[6]) if row[6] else {}
                attributes_list = []
                for key, value in attributes_dict.items():
                    attributes_list.append({"key": key, "value": value})

                # Extract service name from resource
                resource_jsonb = json.loads(row[9]) if row[9] else {}
                service_name = resource_jsonb.get("service.name", "unknown")
